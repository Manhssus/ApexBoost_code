...
namespace Optimizer
{
    static class Program
    {
        ...
        static void Main(string[] switches)
        {
            ...
            Optimizer.Localization.LanguageManager.Initialize();
            ...
            if (!_notRunning)
            {
                MessageBox.Show(_alreadyRunningMsg, "ApexBoost", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Environment.Exit(0);
                return;
            }

            // Load options early to check AlwaysRunAsAdministrator flag without triggering FirstRunForm
            OptionsHelper.LoadSettings();

            if (OptionsHelper.CurrentOptions.AlwaysRunAsAdministrator && !Utilities.IsAdmin())
            {
                try
                {
                ...
                {
                    // User cancelled UAC, continue as standard user
                }
                catch (Exception ex)
                {
                    Logger.LogError("Program", "Auto-elevation failed.", ex.ToString());
                }
            }
            ...
            if (switches.Length == 1)
            {
                ...
                if (arg == "/version")
                {
                    if (!EXPERIMENTAL_BUILD) MessageBox.Show($"Optimizer: {GetCurrentVersionTostring()}\n\nCoded by: deadmoon © ∞\n\nhttps://github.com/hellzerg/optimizer", "ApexBoost", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                if (arg == "/disabledefender")
                {
                    ...
                    MessageBox.Show("Windows Defender has been completely disabled successfully.", "ApexBoost", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Environment.Exit(0);
                    return;
                }
                ...
                if (arg.StartsWith("/config="))
                {
                    ...
                    {
                        MessageBox.Show(_confNotFoundMsg, "ApexBoost", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Environment.Exit(0);
                        return;
                    }
                    ...
                    {
                        MessageBox.Show(_confInvalidFormatMsg, "ApexBoost", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Environment.Exit(0);
                        return;
                    }
                    if (!SilentOps.ProcessWindowsVersionCompatibility())
                    {
                        MessageBox.Show(_confInvalidVersionMsg, "ApexBoost", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Environment.Exit(0);
                        return;
                    }
                    SILENT_MODE = true;
